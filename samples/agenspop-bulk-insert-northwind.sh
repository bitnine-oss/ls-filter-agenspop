...
curl -u $ES_USER:$ES_PASSWORD -X POST "${ES_URI}/${ES_IDX_VERTEX}/_delete_by_query?pretty" -H 'Content-Type: application/json' -d'
{
  "query": {
    "match": {
...
'
sleep 0.5
curl -u $ES_USER:$ES_PASSWORD -X POST "${ES_URI}/${ES_IDX_EDGE}/_delete_by_query?pretty" -H 'Content-Type: application/json' -d'
{
  "query": {
    "match": {
      "datasource": "'${ES_DATASOURCE}'"
...
echo ""
curl -u $ES_USER:$ES_PASSWORD -X GET "${ES_URI}/_cat/indices?v"
